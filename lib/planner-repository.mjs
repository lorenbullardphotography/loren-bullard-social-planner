...
export function createPlannerRepository({ sql, workspaceId = DEFAULT_WORKSPACE_ID } = {}) {
  ...
  // Used only by readSnapshot() for GET /api/planner: "posted" (published,
  // Instagram-synced) assets are always sourced from the legacy document
  // there instead (see that route — only /api/instagram/sync ever writes
  // them, and it writes the legacy document, never row storage), so
  // fetching them here would just be discarded. In real production data
  // that's the vast majority of rows (~980 published posts vs. ~30-70
  // active drafts) — transferring all of them over the pooler on every
  // single page load was a direct, sizeable contributor to Shared Pooler
  // Egress, thrown away immediately after the fetch.
  async function listActiveNonPostedAssets() {
    const rows = await sql`
      SELECT * FROM planner_assets
      WHERE workspace_id = ${workspaceId} AND deleted_at IS NULL AND data ->> 'status' <> 'posted'
      ORDER BY sort_key ASC
    `;
    return rows.map(row => row.data);
  }
  ...
  return {
    ...
    getAssetForUpdate, nextAssetSortKey, insertAssetRow, updateAssetRow, setAssetChangedSequence, softDeleteAssetRow, listActiveAssets, listActiveNonPostedAssets,
