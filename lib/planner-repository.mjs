...
export function createPlannerRepository({ sql, workspaceId = DEFAULT_WORKSPACE_ID } = {}) {
  ...
  async function getSettings() {
    const [row] = await sql`SELECT data FROM planner_settings WHERE workspace_id = ${workspaceId}`;
  ...
  return {
    ensureSchema, health, migrateLegacyPlanner, compareLegacyPlanner, recordMigrationParity, hasVerifiedMigrationParity,
    ...
    getActivityForUpdate, consumeActivityUndo
  };
}
