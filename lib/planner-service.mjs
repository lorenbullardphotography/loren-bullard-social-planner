...
export function createPlannerService({ repository }) {
  ...
  // listActiveNonPostedAssets, not listActiveAssets: GET /api/planner (this
  // function's only caller) always sources "posted" assets from the legacy
  // document instead, so fetching them here too would just be discarded —
  // see that function's comment for why that's most of the table in real
  // production data.
  async function readSnapshot() {
    const posts = await repository.listActiveNonPostedAssets();
  ...
  async function importPlanner({ posts, scratch, settings, actor }) {
    return repository.replaceAllFromImport({ posts, scratch, settings, actor });
  }

  return {
    readSnapshot, createAsset, patchAsset, deleteAsset, reorderAsset, createIdea, patchIdea, deleteIdea, patchSettings,
    latestChangeToken, changesSince, undoActivity, recentActivity, importPlanner
  };
}
